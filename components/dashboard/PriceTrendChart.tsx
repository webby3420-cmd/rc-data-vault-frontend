...
export default function PriceTrendChart({ data }: { data: DataPoint[] }) {
  ...
  const points = data.map((d, i) => `${i * w},${100 - ((d.median_price - min) / range) * 80 - 10}`).join(' ')
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className={`text-sm font-medium ${trendColor}`}>{pctChange >= 0 ? '+' : ''}{pctChange.toFixed(1)}%</div>
      </div>
      <svg viewBox="0 0 100 100" className="w-full h-20" preserveAspectRatio="none">
        <polyline fill="none" stroke="#f59e0b" strokeWidth="2" points={points} vectorEffect="non-scaling-stroke" />
      </svg>
      <div className="flex justify-between mt-1 text-xs text-slate-600">
        <span>{earliest.month?.slice(0, 7)}</span>
