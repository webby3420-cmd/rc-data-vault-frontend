...
export const metadata: Metadata = {
  title: "Best RC Cars Under $500 | RC Data Vault",
  description: "Top RC cars under $500 ranked by real sold market data. Compare fair values, price ranges, and market confidence.",
};
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <nav className="mb-6 text-sm text-slate-400">
          ...
          <span className="text-slate-300">Under $500</span>
        </nav>
        <h1 className="mb-2 text-3xl font-semibold text-white">Best RC Cars Under $500</h1>
        <p className="mb-8 text-slate-400">{variants.length} models ranked by real market data</p>

        {variants.length > 0 ? (
          <div className="space-y-4">
            {variants.map((v: any, i: number) => (
              <BestOfCard key={v.variant_slug} variant={v} rank={i + 1} signupSource="best_of_cars_under_500" imageUrl={imageMap[v.variant_slug]} />
            ))}
          </div>
        ) : (
