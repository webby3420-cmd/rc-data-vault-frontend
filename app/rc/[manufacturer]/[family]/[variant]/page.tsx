...
import { VariantNextStepCta } from '@/components/variant/VariantNextStepCta'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: baseTitle, description: baseDescription, url: data.seo.canonical_url, siteName: "RC Data Vault", type: "website" },
    twitter: { card: "summary", title: baseTitle, description: baseDescription }
  };
}
...
export default async function VariantPage({ params }: PageProps) {
  ...
  const [{ data: payload }, { data: dashboard }] = await Promise.all([
    supabase.rpc("get_variant_page_payload", { p_variant_slug: variantSlug }),
    supabase.rpc("get_variant_dashboard", { p_variant_slug: variantSlug }),
  ]);
  if (!payload) {
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <header className="mb-8">
          <h1 className="text-4xl font-semibold tracking-tight text-white sm:text-5xl">{identity.variant_full_name}</h1>
          <p className="mt-3 text-lg text-slate-300">Current valuation, sold comps, price trends, specs, and market intelligence.</p>
        </header>
        <div className="grid gap-8">
          ...
          {specRows.length > 0 && (<Collapsible title="Specifications"><dl className="grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">{specRows.map(({ label, value }) => (<div key={label}><dt className="text-xs uppercase tracking-wide text-slate-500">{label}</dt><dd className="mt-0.5 text-sm font-medium text-slate-200">{value}</dd></div>))}</dl>{specs?.spec_notes && <p className="mt-4 text-xs text-slate-500">* {specs.spec_notes}</p>}</Collapsible>)}
          {hasValuation ? (<section className="rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-sm"><div className="mb-2 text-sm uppercase tracking-wide text-slate-400">Estimated Value</div><div className="text-5xl font-semibold text-amber-400">{fmt(valuation.estimated_value_mid)}</div><div className="mt-3 text-lg text-slate-200">Range: {fmt(valuation.estimated_value_low)} – {fmt(valuation.estimated_value_high)}</div><div className="mt-3 flex flex-wrap gap-3 text-sm text-slate-300"><span className="rounded-full border border-slate-600 px-3 py-1">{valuation.confidence_label}</span><span>{valuation.observation_count} sold listings</span><span>Updated {fmtDate(valuation.valuation_last_updated_at)}</span></div><div className="mt-2 text-xs text-slate-500">Source: eBay sold listings</div></section>) : (<section className="rounded-2xl border border-slate-700 bg-slate-900 p-6"><div className="text-slate-400">We're actively tracking market activity for this variant. As more verified sales data becomes available, a valuation range and pricing trends will appear here.</div></section>)}
          <PriceAlertSignup
            variantId={variantId}
            variantSlug={variantSlug}
            modelName={identity.variant_full_name}
          />
          {dashboard?.found && <VariantDashboard data={dashboard} />}
          {hasTrends && (<Collapsible title="Market Trend"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm text-slate-200"><thead className="text-slate-400"><tr><th className="pb-3 pr-4">Month</th><th className="pb-3 pr-4">Median</th><th className="pb-3 pr-4">Range</th><th className="pb-3">Sales</th></tr></thead><tbody>{price_trends.map((t: any) => (<tr key={t.month} className="border-t border-slate-800"><td className="py-3 pr-4">{fmtMonth(t.month)}</td><td className="py-3 pr-4 font-medium text-amber-400">{fmt(t.median_price)}</td><td className="py-3 pr-4">{fmt(t.min_price)} – {fmt(t.max_price)}</td><td className="py-3">{t.observation_count}</td></tr>))}</tbody></table></div>{trendDelta !== null && firstTrend && lastTrend && (<p className="mt-3 text-sm text-slate-400">{trendDelta >= 0 ? "↑" : "↓"} Median price {trendDelta >= 0 ? "up" : "down"} {fmt(Math.abs(trendDelta))} from {fmtMonth(firstTrend.month)} to {fmtMonth(lastTrend.month)}</p>)}</Collapsible>)}
          {recent_sales && recent_sales.length > 0 && (<Collapsible title="Recent Sold Listings"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm text-slate-200"><thead className="text-slate-400"><tr><th className="pb-3 pr-4">Price</th><th className="pb-3 pr-4">Date</th><th className="pb-3 pr-4">Source</th><th className="pb-3">Title</th></tr></thead><tbody>{recent_sales.map((s: any, i: number) => (<tr key={i} className="border-t border-slate-800"><td className="py-3 pr-4 font-medium text-amber-400">{fmt(s.price)}</td><td className="py-3 pr-4">{fmtDate(s.price_date)}</td><td className="py-3 pr-4 uppercase text-slate-400">{s.source}</td><td className="py-3">{s.title ? s.title.slice(0, 60) + (s.title.length > 60 ? "…" : "") : "—"}</td></tr>))}</tbody></table></div></Collapsible>)}
          {parts.length > 0 && (<section className="rounded-2xl border border-slate-700 bg-slate-900 p-6"><div className="flex items-center justify-between mb-4"><div><h2 className="text-2xl font-semibold text-white">Parts & Upgrades</h2><p className="mt-1 text-sm text-slate-400">{parts.length} part{parts.length !== 1 ? "s" : ""} available{hasOemParts && hasAftermarketParts && " — OEM + aftermarket"}{hasOemParts && !hasAftermarketParts && " — OEM replacement parts"}</p></div><a href={`/rc/${identity.manufacturer_slug}/${identity.model_family_slug}/${variantSlug}/parts`} className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2 text-sm font-medium text-slate-950 transition hover:bg-amber-400">View all parts</a></div><div className="grid gap-3">{parts.slice(0, 3).map((part: any) => (<div key={part.part_id} className="rounded-xl border border-slate-800 bg-slate-950 p-3 flex items-center justify-between"><div><span className="text-sm text-white">{part.part_name}</span>{part.part_number && <span className="ml-2 text-xs text-slate-500">#{part.part_number}</span>}</div>{part.best_price && <span className="text-sm font-semibold text-amber-400">${Number(part.best_price).toFixed(2)}</span>}</div>))}</div>{parts.length > 3 && <p className="mt-3 text-xs text-slate-500">{parts.length - 3} more parts available →</p>}</section>)}
          {content?.buying_tips && content.buying_tips.length > 0 && (<Collapsible title="What to Look For When Buying Used"><ul className="space-y-3">{content.buying_tips.map((tip: string, i: number) => (<li key={i} className="flex gap-3 text-slate-200"><span className="mt-0.5 flex-shrink-0 text-amber-400 font-semibold">{i + 1}.</span><span className="leading-7">{tip}</span></li>))}</ul></Collapsible>)}
          {hasValuation && (<Collapsible title="Market Summary"><p className="max-w-3xl text-base leading-7 text-slate-200">The {identity.variant_full_name} has a <strong className="text-white">{market_summary.market_depth_label}</strong> secondary market with <strong className="text-white">{valuation.observation_count}</strong> recent sold listings. Prices currently range from <strong className="text-white">{fmt(valuation.estimated_value_low)}</strong> to <strong className="text-white">{fmt(valuation.estimated_value_high)}</strong>, and the market is trending <strong className="text-white">{market_summary.trend_direction?.replace("_", " ")}</strong>.</p><div className="mt-6 grid gap-4 sm:grid-cols-3"><div className="rounded-xl border border-slate-800 p-4"><div className="text-sm text-slate-400">Depth</div><div className="mt-1 text-lg font-medium text-white">{market_summary.market_depth_label}</div></div><div className="rounded-xl border border-slate-800 p-4"><div className="text-sm text-slate-400">Trend</div><div className="mt-1 text-lg font-medium text-white">{market_summary.trend_direction?.replace("_", " ")}</div></div><div className="rounded-xl border border-slate-800 p-4"><div className="text-sm text-slate-400">Data points</div><div className="mt-1 text-lg font-medium text-white">{valuation.observation_count}</div></div></div></Collapsible>)}
          {related?.siblings?.length > 0 && (<Collapsible title="Related Models"><div className="mb-6 grid gap-3">{related.siblings.map((s: any) => (<a key={s.variant_id} className="rounded-xl border border-slate-800 p-4 transition hover:border-slate-600" href={s.canonical_url}><div className="font-medium text-white">{s.full_name}</div><div className="text-sm text-slate-400">{s.obs_count} sold listings</div></a>))}</div><div className="flex flex-wrap gap-3 text-sm"><a className="rounded-full border border-slate-700 px-3 py-2 text-slate-200 hover:border-slate-500" href={related.model_family.canonical_url}>{related.model_family.name} family</a><a className="rounded-full border border-slate-700 px-3 py-2 text-slate-200 hover:border-slate-500" href={related.manufacturer.canonical_url}>{related.manufacturer.name}</a></div></Collapsible>)}
          <div className="mt-8 space-y-6">
            <MoreFromManufacturer
              manufacturerName={identity.manufacturer_name}
              currentCanonicalPath={identity.canonical_url}
              currentFamilySlug={family}
            />
            <VariantNextStepCta
              manufacturerName={identity.manufacturer_name}
              manufacturerSlug={identity.manufacturer_slug}
              modelFamilyName={identity.model_family_name}
              modelFamilySlug={identity.model_family_slug}
              fullName={identity.variant_full_name}
            />
            <RecentlyViewedVariants
              canonicalPath={identity.canonical_url}
              fullName={identity.variant_full_name}
              manufacturerName={identity.manufacturer_name}
            />
          </div>
