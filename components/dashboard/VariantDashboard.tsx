  ...
  return (
    <section className="rounded-2xl border border-slate-700 bg-slate-900 p-6 mt-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 mb-6">
        {collectibility?.score != null && <ScoreCard label="Collectibility" score={collectibility.score} tier={collectibility.label ?? ''} />}
        {demand?.score != null && <ScoreCard label="Demand" score={demand.score} tier={demand.label ?? ''} />}
      ...
      {price_trends?.length >= 2 && <div className="mb-4"><PriceTrendChart data={price_trends} /></div>}
      {buyer_intel?.timing_signal && (
        <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 text-sm">
          ...
          {buyer_intel.recommendation && <p className="text-slate-400 mt-1 text-xs">{buyer_intel.recommendation}</p>}
        </div>
      )}
    </section>
  )
