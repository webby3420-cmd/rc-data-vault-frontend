...
export const metadata: Metadata = {
  title: "Best RC Cars for Beginners | RC Data Vault",
  description: "Beginner-friendly RC cars ranked by real sold market data. Affordable, well-supported models with strong resale value.",
};
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <nav className="mb-6 text-sm text-slate-400">
          ...
          <span className="text-slate-300">For Beginners</span>
        </nav>
        <h1 className="mb-2 text-3xl font-semibold text-white">Best RC Cars for Beginners</h1>
        <p className="mb-8 text-slate-400">{variants.length} beginner-friendly models ranked by real market data</p>

        {variants.length > 0 ? (
          <div className="space-y-4">
            {variants.map((v: any, i: number) => (
              <BestOfCard key={v.variant_slug} variant={v} rank={i + 1} signupSource="best_of_beginners" imageUrl={imageMap[v.variant_slug]} />
            ))}
          </div>
        ) : (
