...
export const metadata: Metadata = {
  title: "Best RC Cars for Resale Value | RC Data Vault",
  description: "RC cars with the strongest resale values ranked by real sold market data. High confidence, data-backed rankings.",
};
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        <nav className="mb-6 text-sm text-slate-400">
          ...
          <span className="text-slate-300">Resale Value</span>
        </nav>
        <h1 className="mb-2 text-3xl font-semibold text-white">Best RC Cars for Resale Value</h1>
        <p className="mb-8 text-slate-400">{variants.length} models with the strongest data-backed resale values</p>

        {variants.length > 0 ? (
          <div className="space-y-4">
            {variants.map((v: any, i: number) => (
              <BestOfCard key={v.variant_slug} variant={v} rank={i + 1} signupSource="best_of_resale" imageUrl={imageMap[v.variant_slug]} />
            ))}
          </div>
        ) : (
